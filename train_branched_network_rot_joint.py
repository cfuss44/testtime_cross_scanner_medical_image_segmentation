...
def run_training(continue_run):
    ...
    with tf.Graph().as_default():
        ...
        loss_total = exp_config.lambda_seg * loss_segmentation + exp_config.lambda_ss * loss_self_supervised

        # ================================================================
        # Add optimization ops.
        # ================================================================
        train_op_train_time = model.training_step(loss_total, segmentation_vars + self_supervised_vars + test_time_opt_vars, exp_config.optimizer_handle_training, learning_rate_pl)
        ...
        for epoch in range(exp_config.max_epochs):
            ...
            for batch in iterate_minibatches(imtr, gttr, batch_size = exp_config.batch_size):
                ...
                # update vars
                # ===========================
                _, loss_value = sess.run([train_op_train_time, loss_total], feed_dict=feed_dict)
                ...
                if step % exp_config.train_eval_frequency == 0:
                    ...
                    [train_loss_seg, train_dice_seg, train_loss_ss, train_acc_ss] = do_eval(sess,
                ...
                if step % exp_config.val_eval_frequency == 0:
                    ...
                    [val_loss_seg, val_dice_seg, val_loss_ss, val_acc_ss] = do_eval(sess,
                                                        eval_loss_segmentation,
                                                        eval_loss_self_supervised,
                                                        images_pl,
                                                        labels_segmentation_pl,
                    ...
                    # ===========================
                    avg_val_da = (val_dice_seg + val_acc_ss) / 2
                    if avg_val_da > best_da:
                        best_da = avg_val_da
                        best_file = os.path.join(log_dir, 'models/best_dice.ckpt')
                        saver_best_da.save(sess, best_file, global_step=step)
                        logging.info('Found new average best dice on validation sets! - %f -  Saving model.' % avg_val_da)
