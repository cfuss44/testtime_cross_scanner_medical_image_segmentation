...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        labels_tensor_shape_self_supervised = [exp_config.batch_size] + list(exp_config.image_size) + [1]
        labels_segmentation_pl = tf.compat.v1.placeholder(tf.uint8, shape=labels_tensor_shape_segmentation, name = 'labels_segmentation')
        labels_self_supervised_pl = tf.compat.v1.placeholder(tf.float32, shape=labels_tensor_shape_self_supervised, name = 'labels_self_supervised')
        ...
        # ================================================================
        logits_self_supervised = exp_config.model_handle_autoencode(image_normalized, training_pl)
        ...
        # ================================================================
        loss_self_supervised = exp_config.loss_handle_autoencode(labels_self_supervised_pl, logits_self_supervised)
        tf.compat.v1.summary.scalar('loss_autoencode', loss_self_supervised)
        ...
        eval_loss_self_supervised = model.evaluation_autoencode(logits_self_supervised, labels_self_supervised_pl)
        ...
        model_path = os.path.join(sys_config.log_root, 'Initial_training_autoencoder')
        checkpoint_path = utils.get_latest_model_checkpoint_path(model_path, 'models/best_dice.ckpt')
        logging.info('Restroring session from: %s' %checkpoint_path)
