...
def training_step(loss, var_list, optimizer_handle, learning_rate, **kwargs):
    ...
    opt_memory_update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
...
# ================================================================
# ================================================================
def evaluation_rotate(logits, labels):
    '''
    A function to compute the loss measures for the rotation task to compare the prediction and the ground truth
    '''
   
    curr_loss = tf.reduce_mean(exp_config.loss_handle_self_supervised(labels, logits))
    curr_acc = exp_config.eval_handle_self_supervised(labels, tf.nn.softmax(logits))

    return curr_loss, curr_acc

# ================================================================
# ================================================================
def evaluation_autoencode(logits, labels):
    '''
    A function to compute the loss measures for the autoencoder to compare the prediction and the ground truth
    '''
   
    curr_loss = exp_config.loss_handle_autoencode(labels, logits)
    curr_acc = exp_config.eval_handle_autoencode(labels, logits)
