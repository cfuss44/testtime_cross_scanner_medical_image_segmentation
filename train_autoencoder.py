...
def run_training(continue_run):
    ...
    with tf.Graph().as_default():
        ...
        labels_tensor_shape_autoencode = [exp_config.batch_size] + list(exp_config.image_size) + [1]
        labels_autoencode_pl = tf.compat.v1.placeholder(tf.float32, shape=labels_tensor_shape_autoencode, name = 'labels_segmentation')
        ...
        # Define the segmentation network here
        # ================================================================
        logits_autoencode = exp_config.model_handle_autoencode(images_pl, training_pl)
        ...
        # ================================================================
        loss_autoencode = exp_config.loss_handle_autoencode(labels_autoencode_pl, logits_autoencode)
        tf.compat.v1.summary.scalar('loss_autoencode', loss_autoencode)
        
        # ================================================================
        # Add optimization ops.
        # ================================================================
        train_op_train_time = model.training_step(loss_autoencode, self_supervised_vars, exp_config.optimizer_handle_training, learning_rate_pl)
        
        # ================================================================
        # Add ops for model evaluation
        # ================================================================
        eval_loss_segmentation = model.evaluation_autoencode(logits_autoencode, labels_autoencode_pl)
        ...
        for epoch in range(exp_config.max_epochs):
            ...
            for batch in iterate_minibatches(imtr, gttr, batch_size = exp_config.batch_size):
                ...
                # update vars
                # ===========================
                _, loss_value = sess.run([train_op_train_time, loss_autoencode], feed_dict=feed_dict)
                ...
                if step % exp_config.val_eval_frequency == 0:
                    ...
                    # ===========================
                    if val_loss_ss < best_da:
                        best_da = val_loss_ss
                        best_file = os.path.join(log_dir, 'models/best_dice.ckpt')
                        saver_best_da.save(sess, best_file, global_step=step)
                        logging.info('Found new average best dice on validation sets! - %f -  Saving model.' % val_loss_ss)
...
def do_eval(sess,
    ...
    :param eval_loss: The placeholder containing the eval loss
    :param images_placeholder: Placeholder for the images
    :param labels_placeholder: Placeholder for the labels
    :param training_time_placeholder: Placeholder toggling the training/testing mode. 
    ...
    for batch in iterate_minibatches(images, labels, batch_size=batch_size):
        ...
        closs, cacc = do_eval_single(sess,
                                    eval_loss,
                                    images_placeholder,
                                    labels_placeholder,
                                    training_time_placeholder,
    ...
    avg_dice = acc_ii / num_batches

    logging.info('  Average loss: %0.04f, average accuracy: %0.04f' % (avg_loss_seg, avg_dice))
